...
struct EuclideanMetricPbc {
  ...
  EuclideanMetricPbc(Vector dims, std::vector<bool> p) :
      dimensions(dims), periodic(p) {}

  double operator()(const Container& v1, const Container& v2) const {
    Container diff(v1.size());
...
public:
  ...
  explicit Searcher(const VpTree* tree, const Vector& target, double radius);
...
DistancesIndices VpTree::getNeighborhoodPoints(const Vector& target, double radius) const {
  auto targetDimension = target.size();
  if (targetDimension != dimension_) {
    throw DimensionMismatch(dimension_, targetDimension);
  }
...
Searcher::Searcher(const VpTree* tree, const Vector& target, double radius) :
